...
const playerFields = z.object({ name: z.string().trim().min(2).max(100), lastname: z.string().trim().max(100).default(''), nickname: z.string().trim().min(2).max(60), avatar: z.string().url().optional(), sport: z.string().trim().min(2).max(80).default('Esports'), position: z.string().trim().min(2).max(80).default('Jugador'), nationality: z.string().trim().min(2).max(60).default('MX'), status: z.enum(['active', 'inactive', 'suspended', 'ACTIVE', 'OFFLINE', 'SUSPENDED']).default('active') }).passthrough();
