...
const teamFields = z.object({ name: z.string().trim().min(2).max(60), abbreviation: z.string().trim().min(2).max(8).optional(), tag: z.string().trim().min(2).max(8).optional(), logo: z.string().url().max(500).optional(), sport: z.string().trim().min(2).max(80).default('Esports'), region: z.string().trim().min(2).max(80).default('LATAM'), competitionType: z.string().trim().min(2).max(80).default('Regional'), description: z.string().trim().max(300).default(''), status: z.enum(['active', 'inactive', 'draft']).default('active') }).passthrough();
