...
export const TeamsListView: React.FC<TeamsListViewProps> = ({
  ...
  const handleCreateTeam = (e: React.FormEvent) => {
    ...
    if (formName.trim().length < 2 || formName.trim().length > 20 || formTag.trim().length < 2 || formTag.trim().length > 8 || formDescription.trim().length > 300 || formLogo.trim().length > 500) {
      notify('error', 'Revisa los límites: nombre 2-20, tag 2-8, descripción 300 y URL 500 caracteres.');
      return;
    }
          ...
          <div className="bg-[#12141e] border border-[#282e44] rounded-3xl p-6 sm:p-8 max-w-lg w-full space-y-6 shadow-2xl">
            ...
            <form onSubmit={handleCreateTeam} className="space-y-4">
              <div>
                ...
                />
                <p className="mt-1 text-right text-[10px] text-slate-500">{formName.length}/20 caracteres</p>
              </div>
