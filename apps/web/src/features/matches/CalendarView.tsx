...
export const CalendarView: React.FC<CalendarViewProps> = ({ onOpenMatch }) => {
  ...
  useEffect(() => {
    ...
    const loadMatches = async () => {
      try {
        setIsLoading(true);
        setError('');
        const [nextMatches, nextTeams] = await Promise.all([tournamentXApi.matches(), tournamentXApi.teams()]);
        if (!active) return;
  ...
  return (
    <div id="calendar-view" className="p-6 lg:p-8 max-w-7xl mx-auto space-y-7">
      ...
      {!isLoading && !error && visible.length === 0 && <p className="rounded-xl border border-[#252a3d] bg-[#11131c] p-5 text-sm text-slate-400">No hay partidos que coincidan con los filtros.</p>}
