...
export const LandingView: React.FC<LandingViewProps> = ({ onEnterApp, onOpenCreateWizard, onOpenAuth, teams, tournaments }) => {
  ...
  <div className="tx-module-shell min-h-screen bg-[#0b0c10] text-white">
    ...
    <main>
      <section className="tx-landing-hero max-w-7xl mx-auto px-6 py-20 lg:py-28 grid lg:grid-cols-[1.1fr_.9fr] gap-14 items-center">
        <div>
          <p className="tx-kicker mb-5">Deportes y esports en un solo lugar</p>
          <h1 className="tx-hero-title font-brand text-6xl sm:text-7xl lg:text-8xl font-black uppercase">Organiza.<br />Compite.<br /><span className="text-[#ff2e83]">Gana.</span></h1>
        ...
        <div className="tx-spotlight rounded-3xl border border-[#8b5cf6]/25 bg-[radial-gradient(circle_at_100%_0,rgba(139,92,246,.16),transparent_38%),radial-gradient(circle_at_0_100%,rgba(79,124,255,.1),transparent_38%),#111218] p-7">
          ...
          <div className="grid grid-cols-2 gap-3 py-6 border-y border-white/10"><div className="rounded-2xl bg-black/20 p-4"><p className="text-[10px] uppercase tracking-wider text-slate-500">Inscripciones</p><strong className="mt-2 block text-2xl">{featured.registeredTeams}/{featured.maxTeams}</strong></div><div className="rounded-2xl bg-black/20 p-4"><p className="text-[10px] uppercase tracking-wider text-slate-500">Premio</p><strong className="mt-2 block text-2xl text-[#d6b15e]">{featured.prizePool || `$${featured.prizeAmountUSD.toLocaleString()}`}</strong></div></div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="flex items-end justify-between mb-9"><div><p className="text-sm text-[#ff2e83] font-semibold">Circuito TournamentX</p><h2 className="mt-2 text-3xl font-bold">Competencias registradas</h2></div><button onClick={() => onEnterApp('tournaments')} className="text-sm text-slate-400 hover:text-white">Ver todas</button></div>
