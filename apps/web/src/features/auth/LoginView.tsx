...
export const LoginView: React.FC<LoginViewProps> = ({ onAuthenticated, onBackToHome }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (mode === 'register') {
      if (!/^[\p{L}][\p{L}\s.'-]*$/u.test(normalizedName) || normalizedName.length < 2 || normalizedName.length > 30) {
        setError('Escribe un nombre de 2 a 30 caracteres usando solo letras, espacios, guiones, puntos o apóstrofes.');
        return;
      }
      if (normalizedEmail.length > 30) {
        setError('El correo electrónico debe tener como máximo 30 caracteres.');
        return;
      }
      if (password.length > 20) {
        setError('La contraseña debe tener como máximo 20 caracteres.');
        return;
      }
      ...
      <section className="p-6 sm:p-12 flex items-center justify-center">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            {mode === 'register' && <label className="block"><span className="block mb-2 text-sm text-slate-300">Nombre completo</span><div className="relative"><UserPlus className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" /><input required minLength={2} maxLength={30} value={name} onChange={(event) => setName(event.target.value.replace(/[^\p{L}\s.'-]/gu, ''))} className="field pl-11" placeholder="Ej. Ana López" autoComplete="name" /></div><small className="mt-2 block text-[11px] text-slate-500">De 2 a 30 caracteres: letras, espacios, guiones, puntos o apóstrofes.</small></label>}
            <label className="block"><span className="block mb-2 text-sm text-slate-300">Correo electrónico</span><div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" /><input type="email" required maxLength={30} value={email} onChange={(event) => setEmail(event.target.value.trimStart().toLowerCase())} className="field pl-11" autoComplete="email" inputMode="email" placeholder="nombre@correo.com" /></div>{mode === 'register' && <small className="mt-2 block text-[11px] text-slate-500">Usa un correo válido, por ejemplo nombre@correo.com (máximo 30 caracteres).</small>}</label>
            <label className="block"><span className="block mb-2 text-sm text-slate-300">Contraseña</span><div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" /><input type="password" required minLength={8} maxLength={20} {...(mode === 'register' ? { pattern: '(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{8,}', title: 'Usa entre 8 y 20 caracteres, una mayúscula, una minúscula y un número' } : {})} value={password} onChange={(event) => setPassword(event.target.value)} className="field pl-11" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} /></div>{mode === 'register' && <small className="mt-2 block text-[11px] text-slate-500">De 8 a 20 caracteres, con mayúscula, minúscula y número.</small>}</label>
            {mode === 'register' && <label className="block"><span className="block mb-2 text-sm text-slate-300">Confirmar contraseña</span><div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" /><input type="password" required minLength={8} maxLength={20} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="field pl-11" autoComplete="new-password" aria-invalid={Boolean(confirmPassword && password !== confirmPassword)} /></div>{confirmPassword && <small className={`mt-2 block text-[11px] ${password === confirmPassword ? 'text-emerald-400' : 'text-red-300'}`}>{password === confirmPassword ? 'Las contraseñas coinciden.' : 'Las contraseñas no coinciden.'}</small>}</label>}
