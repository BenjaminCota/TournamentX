...
export const LoginView: React.FC<LoginViewProps> = ({ onAuthenticated }) => {
  ...
  const [email, setEmail] = useState('admin@tournamentx.local');
  ...
  return (
    <main className="min-h-screen bg-[#08090d] text-white grid lg:grid-cols-[1.05fr_.95fr]">
      <section className="hidden lg:flex relative overflow-hidden p-14 border-r border-white/10 flex-col justify-between bg-[#0e1017]">
        ...
        <div className="relative max-w-lg"><span className="inline-flex px-3 py-1 rounded-full border border-[#ff2e83]/30 bg-[#ff2e83]/10 text-[#ff69a8] text-xs font-bold tracking-[.2em]">CONTROL TOTAL DEL TORNEO</span><h1 className="font-brand text-7xl font-black uppercase leading-[.9] mt-6">Compite.<br/><span className="text-[#ff2e83]">Organiza.</span><br/>Domina.</h1><p className="mt-6 text-slate-400 max-w-md">Identidad, torneos, resultados en vivo, streams y premios desde una sola plataforma local.</p></div>
        <div className="relative grid grid-cols-3 gap-3">{['8 módulos','Tiempo real','Datos locales'].map((item) => <div key={item} className="rounded-xl border border-white/10 bg-white/[.03] p-3 text-xs text-slate-300"><CheckCircle2 className="w-4 h-4 text-[#ff2e83] mb-2" />{item}</div>)}</div>
      </section>
      <section className="p-6 sm:p-12 flex items-center justify-center">
        <div className="w-full max-w-md">
          ...
          <p className="mt-2 text-sm text-slate-500">{mode === 'login' ? 'Tu rol y permisos se cargan de forma segura.' : 'La cuenta nueva inicia como Espectador.'}</p>
          ...
          {mode === 'login' && <div className="mt-7 pt-6 border-t border-white/10"><p className="text-[11px] uppercase tracking-[.18em] text-slate-500 mb-3">Accesos locales de prueba</p><div className="grid grid-cols-2 gap-2">{demoAccounts.map((account) => <button type="button" key={account.email} onClick={() => { setEmail(account.email); setPassword(account.password); }} className="rounded-lg border border-white/10 bg-white/[.03] px-3 py-2 text-left text-xs text-slate-300 hover:border-[#ff2e83]/50 hover:text-white">{account.label}</button>)}</div></div>}
        </div>
      </section>
    </main>
  );
