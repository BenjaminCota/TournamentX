...
export const UsersView: React.FC<UsersViewProps> = ({ currentUserRole, currentUserId }) => {
  ...
  const load = async () => {
    ...
    const errors = [accounts, organizerRequests]
      .filter((result) => result.status === 'rejected')
      .map((result) => result.status === 'rejected' && result.reason instanceof Error ? result.reason.message : 'No se pudo cargar una sección');
