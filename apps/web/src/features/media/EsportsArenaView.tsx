...
export const EsportsArenaView: React.FC<EsportsArenaViewProps> = ({ currentUserRole }) => {
  ...
  const [integration, setIntegration] = useState({ twitch: 'demo', youtube: 'demo' });
  ...
  const displayedViewers = selectedEvent ? Math.max(0, selectedEvent.viewers + Math.round(Math.sin(tick / 7) * 42)) : 0;
  ...
  return (
    <div className="min-h-screen bg-[#0f0f12] px-4 py-6 text-[#f7f7f8] sm:px-6 lg:px-8">
              ...
              <div className="grid max-h-80 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
                {streams.map((stream) => {
                  ...
                  return <button type="button" key={stream.id} onClick={() => selectStream(stream)} className={`group flex min-w-0 items-center gap-3 rounded-xl border p-3 text-left transition-all ${active ? 'border-[#d6b15e]/50 bg-[#d6b15e]/[.08]' : 'border-white/[.07] bg-black/20 hover:border-white/20 hover:bg-white/[.04]'}`}>
                    ...
                    <span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-white">{stream.title}</span><span className="mt-1 block truncate text-[10px] text-slate-500">{stream.platform} · {stream.channel} · {compactNumber(stream.viewers)}</span></span>
          ...
          <aside className="overflow-hidden rounded-2xl border border-white/[.08] bg-[#18181c] shadow-2xl shadow-black/20">
            {selectedEvent ? <>
              <div className="border-b border-white/[.07] p-5">
                <div className="flex items-center justify-between gap-3"><span className="rounded-md bg-red-500/15 px-2 py-1 text-[10px] font-black tracking-wider text-red-300">● EN VIVO</span><span className="rounded-md border border-white/[.08] px-2 py-1 text-[10px] uppercase tracking-wider text-slate-500">{selectedEvent.category === 'esports' ? 'Esports' : 'Deportes'}</span></div>
              ...
              <div className="p-5">
                ...
                <div className="mt-6 grid grid-cols-2 gap-2">
                  <div className="rounded-xl border border-white/[.06] bg-black/20 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-600">{selectedEvent.clockLabel}</p><p className="mt-1 font-mono text-lg font-semibold tabular-nums">{formatClock(selectedEvent.elapsedSeconds + tick)}</p></div>
                  <div className="rounded-xl border border-white/[.06] bg-black/20 p-3"><p className="text-[10px] uppercase tracking-wider text-slate-600">Audiencia</p><p className="mt-1 text-lg font-semibold tabular-nums">{compactNumber(displayedViewers)}</p></div>
                </div>
              ...
              <div className="flex items-center gap-2 border-t border-white/[.07] bg-black/20 px-5 py-3 text-[10px] text-slate-500"><Sparkles className="h-3.5 w-3.5 text-[#d6b15e]"/> Datos demo dinámicos con estructura de proveedor real</div>
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.65fr)]">
          <div className="overflow-hidden rounded-2xl border border-white/[.08] bg-[#18181c]">
            <div className="flex flex-col gap-4 border-b border-white/[.07] p-5 sm:flex-row sm:items-center sm:justify-between">
              <div><h2 className="flex items-center gap-2 text-base font-semibold"><CalendarClock className="h-4 w-4 text-[#d6b15e]"/> Eventos destacados</h2><p className="mt-1 text-xs text-slate-500">Referencias reales con marcador simulado para esta demo local.</p></div>
